...
export default async function trainingRoutes(app: FastifyInstance) {
  ...
  app.get("/", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    const trainings = await listTrainings({ categoryId: query.categoryId, publishedOnly }, request.user.sub);
  ...
  app.get("/:id", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    const publishedOnly = request.user.role !== "ADMIN";
    const training = await getTrainingById(id, publishedOnly, request.user.sub);
    reply.send({ training });
  });
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const training = await createTraining(input, request.user.sub);
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      const module = await updateModule(moduleId, input);
      reply.send({ module });
    }
  );
  ...
  app.post("/lessons/:lessonId/progress", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    await setLessonProgress(request.user.sub, lessonId, input.completed);
    reply.code(204).send();
  });
}
