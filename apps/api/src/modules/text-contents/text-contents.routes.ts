...
export default async function textContentRoutes(app: FastifyInstance) {
  ...
  app.get("/", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    const result = await listTextContents({ ...query, publishedOnly }, pagination);
    reply.send(result);
  });
  ...
  app.get("/:id", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    const publishedOnly = request.user.role !== "ADMIN";
    const content = await getTextContentById(id, publishedOnly);
    reply.send({ content });
  });
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const content = await createTextContent(input, request.user.sub);
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      const content = await updateTextContent(id, input);
      reply.send({ content });
    }
  );
