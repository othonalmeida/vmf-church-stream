...
export function AdminSidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-60 flex-col border-r border-surface-border bg-surface-raised md:flex">
      <div className="flex items-center gap-2 border-b border-surface-border px-5 py-4 text-white">
        ...
        {ADMIN_NAV.map((item, idx) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "mx-2 mb-1 flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-white/70 hover:bg-surface-border hover:text-white",
      ...
      <Link
        href="/browse"
        className="flex items-center gap-2 border-t border-surface-border px-5 py-4 text-xs text-white/50 hover:text-white"
      >
